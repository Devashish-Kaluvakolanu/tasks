...
export function injectPositive(values: number[]): number[] {
    const valueInside = [...values];
    const firstNegativeIndex = valueInside.findIndex(
        (val: number): boolean => val < 0,
    );
    if (firstNegativeIndex === -1) {
        const totalSum = valueInside.reduce((acc, num) => acc + num, 0);
        valueInside.splice(valueInside.length, 0, totalSum);
    } else {
        const sumTillNegative = valueInside
            .slice(0, firstNegativeIndex)
            .reduce((acc, num) => acc + num, 0);
        valueInside.splice(firstNegativeIndex + 1, 0, sumTillNegative);
    }
    return valueInside;
    /*let result: number[] = [];
    let sum: number = 0;
